...
\opt{bibtex}驱动当然是\biblatex/\biber 支持数据格式中最全面和成熟的。\secref{bib:typ:als}和\secref{bib:fld:als}节的别名通过下面这些默认数据源映射实现。
...
\subsection[纪年顺序]{纪年格式}%Chronological Templates
\label{apx:srt:chr}
...
\biblatexml\ XML数据源格式是为\biblatex\ 用户设计的一个可扩展的现代数据源格式。\bibtex\ 格式\file{.bib}文件存在一些局限，特别是涉及到支持UTF-8和姓名格式问题时。\biber\ 通过采用修改的\texttt{btparse} C库等多种方式处理UTF-8方面的限制，但针对\bibtex\ 的相当古老的姓名解析规则是根据简单的西方人名硬编码的。
...
\biblatexml 是为参考文献数据提供的一种XML格式。当\biber\ 读写\biblatexml\ 格式数据源时，它自动为数据源输出一个RelaXNG XML纲要，这一概要由活动的\biblatex\ 数据模型动态生成。\biblatexml\ 数据源因为可用的域依赖于数据模型所以不存在静态的纲要。\biblatexml\ 数据源格式具有相当高的自解析性---要理解其格式，通常只需要从已有的\bibtex\ 格式数据源生成一个\biblatexml\ 数据源，然后分析其内容就可以做到。\biber 也允许用户根据数据模型生成纲要验证\biblatexml\ 数据源。
...
因为\biblatexml\ 格式是 XML并且依赖于数据模型，且数据模型是用户可扩展的(见\secref{aut:ctm:dm})，\biblatexml\ 格式可以处理\bibtex\ 格式数据源无法处理的一些扩展内容比如新的姓名成分，子条目上的选项等。因为它是一种XML 格式，所以使用标准的XML处理库和工具转换成其他XML格式或HTML也相当简单。
...
\end{lstlisting}
%
正文是一个或多个\bibfield{entry}元素存在于顶层的\bibfield{entries}元素中，并且所有都处于\bibfield{bltx}命名空间内。一个条目具有一个\bibfield{id}属性对应于\bibtex\ 条目关键词，具有一个\bibfield{entrytype}属性对应于\bibtex\ 条目类型。例如:
...
\begin{itemize}
  ...
  \item 姓名成分还可以包含下层姓名成分，所以可以处理混合姓名。
...
数据源日期域(见\secref{bib:fld:typ})可以根据是否构成一个日期范围采用两种方式表示:
...
\end{lstlisting}
%
如\secref{aut:ctm:rel}所述，\bibfield{string}和\bibfield{options}和属性是可选的。
...
针对每个条目、每个姓名列表和每个姓名的选项是在数据源中例如\file{.bib}文件中设置的，详见 \biber 文档，这里也给出一些例子：
...
针对姓名列表和单个姓名的选项设置需要使用 \biblatexml 数据源格式或者\biber 支持扩展的\bibtex 姓名格式(详见\biber 文档)，针对姓名列表的选项设置：
...
更新历史是与宏包用户相关的一些更新列表。不影响用户接口或者宏包行为的更技术性的更新并未在列。更多的技术细节可以参考\file{CHANGES.org}文件。右侧的数字表示本手册中的相关节号。
...
\item 增加了 \cmd{DeclareSortInclusion}\see{aut:ctm:srt}
%\cmd{DeclareSortInclusion}\see{aut:ctm:srt}
\item 增加了 \cmd{relateddelim$<$relatedtype$>$}\see{use:fmt:fmt}
\end{release}

\begin{release}{3.6}{2016-09-15}
\item 修正了文档的一些内容，修复了labeldate本地化字符串相关的一个错误
...
\item 修改\cmd{DeclareStyleSourcemap}可多次使用\see{aut:ctm:map}
...
\item 修改日期完全符合\acr{EDTF}0层次和1层次规则，相关判断和本地化字符串\see{bib:use:dat}
...
\item 在\bibtex 数据源中，可以对具体姓名列表和具体姓名设置\opt{sortnamekeyscheme} 和 \opt{useprefix}。
...
\item 新的上下文敏感的分隔符接口\see{use:fmt:csd}
...
\item 增加\gls{参考文献环境}自动分配的新宏\see{use:bib:context}
...
\item 增加\cmd{pernottype}实现的排除具体条目类型的数据源映射\see{aut:ctm:map}
...
\item 数据模型中的姓名列表自动创建\cmd{ifuse$<$name$>$}判断和布尔值\see{use:opt:bib:hyb} and \see{aut:aux:tst}
