...
和其它EDTF level 1允许的格式，见 \secref{bib:use:dat} 节。
...
关键词的分隔值列表。
这些关键词主要用于文献过滤器，通常不会打印出来(见\secref{use:bib:bib, use:use:div} 节)。
...
这在利用文献过滤器创建文献细分时很有用。更多细节请参考 \secref{use:srt} 以及 \secref{aut:ctm:srt} 节。
该域在后端程序处理过程中被清楚，不出现在 \path{.bbl} 中。
...
与本条目关联的其它条目的引用关键字。其关联关系由 \bibfield{relatedtype} 域指定。
...
\bibfield{location} 的别名，用于兼容 \BibTeX 。
传统的 \BibTeX 使用这一稍微有些误导性的域 \bibfield{address} 来表示出版地点，即出版者的所在地，
而 \biblatex 使用更一般的 \bibfield{location} 域。见 \secref{bib:fld:dat,bib:use:and} 节。
...
本宏包考虑到包含一些兼容性代码，用于处理那些由传统 \BibTeX 样式生成的 \file{bib} 文件。
但不幸的是，对所有的老文件都进行自动处理是不可能的，因为 \biblatex 的数据模型与传统的 \BibTeX 有少许不同。
因此，为了能在本宏包中正确使用，这样的 \file{bib} 文件也许需要稍作修改。大体上，下列事项是与传统的 \BibTeX 样式不同的：
...
\sty{natbab} 宏包使用 \bibfield{key} 域作为备用的标注标签，而\biblatex 则使用 \bibfield{label} 域来代替。
...
相比于\acr{ISO8601v2004}允许的有些混乱的格式，\acr{EDTF} 是其中一个更严格的子集，更适用于参考文献数据。

%In addition to the \acr{EDTF} empty date range markers, you may also specify an open ended/start date range by giving the range separator and omitting the end/start date (\eg \texttt{YYYY/}, \texttt{/YYYY}). See \tabref{bib:use:tab1} for some examples of valid date specifications and the formatted dates automatically generated by \biblatex. The formatted date is language specific and will be adapted automatically. If there is no \bibfield{date} field in an entry, \biblatex will also consider the fields \bibfield{year} and \bibfield{month} for backwards compatibility with traditional \bibtex but this is not encouraged as explicit \bibfield{year} and \bibfield{month} are not parsed for date meta-information markers or times and are used as-is.
除了 \acr{EDTF} 空日期范围标记外，还通过给定范围分隔符并省略结束或开始日期的方式（例如 \texttt{YYYY/}、\texttt{/YYYY}）
来指定无末端或无开端的日期范围。\tabref{bib:use:tab1} 列出了一些有效的日期规范以及由 \biblatex 自动生成的日期格式。
...
随着 \file{bib} 文件的处理，所有的日期都被解析并分解为各个日期成分。样式使用日期和时间成分主要通过通过 \secref{aut:bbx:fld:dat} 节讨论的特殊域。更多信息请参考该节和 \pageref{aut:bbx:fld:tab1} 页的\tabref{aut:bbx:fld:tab1}。
...
\acr{EDTF} 通过负日期格式支持公元前（before common era, BCE/BC）日期，此外还支持“近似”（circa）和不确定的日期。
这样的日期格式设置可以检测的内部标记，进而可以插入合适的本地化标记（例如  \opt{circa} 或 \opt{beforecommonera}）。
另外，不确定日期（\acr{EDTF} 5.2.2）会根据\bibfield{\prm{datetype}dateunspecified} 域指定的未定数据间隔尺寸，自动展开成合适的日期范围。
%Styles may use this information to format such dates appropriately but the standard styles do not do this. See \tabref{bib:use:tab3} on page~\pageref{bib:use:tab3} for the allowed \acr{EDTF} <unspecified> formats, their range expansions and \bibfield{$<$datetype$>$dateunspecified} values (see \secref{aut:bbx:fld:gen}).
参考文献样式可以使用该信息构造合适的日期格式，但标准样式不会使用。
\pageref{bib:use:tab3} 页的\tabref{bib:use:tab3} 列出了允许的 \acr{EDTF} 未定日期格式、范围展开和 \bibfield{\prm{datetype}dateunspecified} 域的值（\secref{aut:bbx:fld:gen} 节）。
...
\begin{table}
	...
	\begin{tabularx}{\textwidth}{@{}>{\ttfamily}llX@{}}
		...
		\bottomrule
	\end{tabularx}
	\caption{EDTF 5.2.2 未定日期解析}%Unspecified Date Parsing
	\label{bib:use:tab3}
\end{table}
...
\tabref{bib:use:tab2} 展示了使用恰当测试和格式化的日期格式。
...
另外也支持基于Unicode通用区域数据库（Common Locale Data Repository，\acr{CLDR}）的排序裁剪
