...
该选项控制多语种支持功能。默认情况下，
\biblatex 会通过\sty{babel}/\sty{polyglossia}获取当前环境激活的语言
...
%Specifies the default encoding of the \file{bib} files. This can be overridden on a per-datasource basis using the \opt{bibencoding} option to \cmd{addbibresource}, see \secref{use:bib:res}.  This corresponds to \biber's \opt{--input-encoding} option. The following choices are available:
...
%A separator to be printed after the name used for alphabetizing in the bibliography (\bibfield{author} or \bibfield{editor}, if the \bibfield{author} field is undefined). With the default styles, this separator replaces \cmd{newunitpunct} at this location. The default definition is \cmd{newunitpunct}, \ie it is not handled differently from regular unit punctuation.
...
%Declares \prm{alias} to be an alias for the delimiter \prm{delim}. The assigment is performed for all existing contexts of \prm{alias}.
