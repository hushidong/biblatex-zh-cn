...
\subparagraph{<ibid> styles} 具有<ibid.> 功能的样式，比如 \texttt{authortitle-ibid}, \texttt{author\allowbreakhere title-icomp}, \texttt{author\allowbreakhere year-ibid}, \texttt{authoryear-icomp}, \texttt{ver\allowbreakhere bose-ibid}, \texttt{verbose-inote}, \texttt{verbose-trad1}, \texttt{verbose-trad2} 和 \texttt{verbose-trad3} 支持全局的\opt{ibidpage} 选项.
...
\subparagraph{\texttt{verbose}} 所有\texttt{verbose}类的标注样式提供全局选项 \opt{citepages}.
...
该选项控制包含页码范围前注的完整标注中的\bibfield{page}\slash\bibfield{pagetotal}域的输出。该选项可以用于取消在完整标注中对两个页码范围的跳转，比如下面这样的标注：
...
这里<p.\,125>是\bibfield{postnote}的参数，而<pp.\,100--150>则是\bibfield{pages}域的值。
...
\item[permit] %Allow duplication of page specifications, i.e.\ print both \bibfield{page}\slash\bibfield{pagetotal} and \bibfield{postnote}.
    允许出现两个页码描述，比如既输出\bibfield{page}\slash\bibfield{pagetotal} 也输出 \bibfield{postnote}。
...
\item[suppress] %Unconditionally suppress the \bibfield{pages}\slash \bibfield{pagetotal} fields in citations, regardless of the \bibfield{postnote}.
    无条件取消\bibfield{pages}\slash \bibfield{pagetotal}域的输出，不考虑
    \bibfield{postnote}的情况
...
\item[omit] %Suppress the \bibfield{pages}\slash \bibfield{pagetotal} if the \bibfield{postnote} contains a page range. They are still printed if there is no \bibfield{postnote} or if the \bibfield{postnote} is not a number or range.
    当\bibfield{postnote}包含页码范围时，取消\bibfield{pages}\slash \bibfield{pagetotal} 域的输出。而当\bibfield{postnote}不存在或者不是一个数字或范围时输出。
...
\item[separate] %Separate the \bibfield{pages}\slash \bibfield{pagetotal} from the \bibfield{postnote} if the latter contains a page range. The string \texttt{thiscite} is added to separate the two page ranges.
    当\bibfield{postnote}包含一个页码范围时，则两者均输出，两者之间由\texttt{thiscite}字符串分隔。
\end{valuelist}
...
\subparagraph{\texttt{verbose-trad}} \texttt{verbose-trad}类的标注样式提供了全局选项 \opt{strict}。
...
该选项设置\secref{aut:aux:tst}节的\cmd{if$<$namepart$>$inits}测试。
\texttt{$<$namepart$>$} 是数据模型中由\cmd{DeclareDatamodelConstant}
（见\secref{aut:bbx:drv}）命令定义的任意有效姓名成分。
例如，对于名（given name），该选项就变为\opt{giveninits}。该选项可以针对具体类型、条目、姓名和姓名列表设置。
...
如果\opt{giveninits}设置为\opt{true}，默认的姓名格式将仅提供名的缩写，而不是全名。标准样式仅适用\cmd{ifgiveninits}测试，因此仅响应\opt{giveninits}选项。设置其它姓名成分的选项对默认的姓名格式没有影响。
...
而受必须各自显式给出的\cmd{DeclareSortingNamekeyTemplate}和\opt{uniquename}选项
（或\cmd{DeclareUniquenameTemplate}）的影响。当使用\opt{giveninits}时，且\opt{uniquename}设置为\opt{full}类的值时，会给出警告信息，并自动设置\opt{uniquename}为对应的\opt{init}。
...
噶选项可能在每一条目条目的处理层级最有用，比如一些条目具有\bibfield{volume}域
...
%Whether or not to update the \cnt{uniquename} counter, see \secref{aut:aux:tst} for details. This feature will disambiguate individual names in the \bibfield{labelname} list. This option is also settable on a per-type  and per-entry basis. The possible choices are:
...
Note that the \opt{uniquelist} option will also affect the \cmd{ifsingletitle} test and the \bibfield{extradate} and \bibfield{extraname} fields. See \secref{aut:cav:amb} for furth
...
\opt{uniquelist} 和 \opt{min/maxnames} 选项所产生————会产生不同的姓名列表哈希值（进而产生不同的\opt{extraname} 和 \opt{extradate}值。）和不同的排序。这一选项允许改变这一行为。当选项设置为\prm{true}时，\biber 将忽略在产生姓名列表的哈希值时忽略 <et al> 。考虑：
...
如果设置选项\kvopt{maxnames}{3}, \kvopt{minnames}{1}, \kvopt{nohashothers}{false}，结果为：
...
当选项设置为\kvopt{maxnames}{3}, \kvopt{minnames}{1}, \kvopt{nohashothers}{true}时，结果则变为：
...
注意\opt{nohashothers}选项将影响\bibfield{extradate} 和 \bibfield{extraname} 域。
...
该选项与\opt{nohashothers}选项的效果有些相关，但应用于排序————姓名的可见列表（\opt{minsortnames}的值）用于决定排序是否忽略截短。这意味着，当\kvopt{nosortothers}{true}时，姓名列表：
...
在排序中完全一样的。\opt{nosortothers}选项的默认设置总是以该例中的顺序排列，即截短的姓名列表总是在与其截短点前面内容一致的姓名列表后面。
...
This style also provides an additional preamble option called \opt{subentry} which affects the handling of entry sets. If this option is disabled, citations referring to a member of a set will point to the entire set. If it is enabled, the style supports citations like «[SGW(c)]» which point to a subentry in a set (the third one in this example). See the style example for details.
...
在注解标记后是一个可选的命名了的注解标记
 \footnote{见 \biber 的 \opt{--named-annotation-marker} 选项.}
...
日期域比较特殊，会在\cmd{currentfield} 无法访问的环境中处理，因此需要第4个命令来测试日期的注解。
...
展开为\prm{true}，如果日期域\prm{datetype}存在注解\prm{annotation}，否则为false。如果\prm{annotatationname}未给出，那么会假定为<default>(该名称用来标识未显式给出名称的注解)。 \prm{datetype}参数时必须的，
因为它在大多数使用\cmd{ifdateannotation}的环境中无法推断出来。
...
展开为\prm{true}，如果数据域 \prm{field} 存在文本注解\prm{annotationname} ，否则为false。如果\prm{annotatationname}未给出，那么会假定为<default>(该名称用来标识未显式给出名称的注解)。如果\prm{field}未给出，则当前数据域为 \cmd{currentfield}, \cmd{currentlist} 或 \cmd{currentname}标识的域(见\secref{aut:bib:fmt})。 当然这仅在这些命令已定义情况下才有效，比如在一个格式化命令中。
...
展开为\prm{true}，如果数据域 \prm{field} 内的\prm{item}存在
文本注解\prm{annotationname} ，否则为false。如果\prm{annotatationname}未给出，那么会假定为<default>(该名称用来标识未显式给出名称的注解)。
可选参数\prm{field} 未给出时，可以通过\cmd{iffieldannotation}推断出来。
如果\prm{item}未给出，当前处理的项的数量则由\cnt{listcount}给出。
...
展开为\prm{true}，如果数据域 \prm{field} 内的\prm{item}中的\prm{part}存在
文本注解\prm{annotationname} ，否则为false。如果\prm{annotatationname}未给出，那么会假定为<default>(该名称用来标识未显式给出名称的注解)。
两个可选参数\prm{field}和\prm{item}在\cmd{ifitemannotation}可以推断出来。
\prm{part} 参数无法推断，因此是必须的。
...
获取\prm{field}域的任意文本注解。如果\prm{annotatationname}未给出，
则假定 <default>(这是未显式给出名称的注解名)。如果\prm{field}未给出，
当前数据域假定为\cmd{currentfield}, \cmd{currentlist} 或 \cmd{currentname} (见\secref{aut:bib:fmt})表示的域。当然，这仅在这些命令定义的情况下才有效，比如在格式化命令中。
...
获取\prm{field}域\prm{item} 项的任意文本注解。如果\prm{annotatationname}未给出，
则假定 <default>(这是未显式给出名称的注解名)。如果可选参数\prm{field}未给出，
可以通过\cmd{getfieldannotation}推断出来。如果\prm{item} 未给出，则当前处理的项的序号\cnt{listcount} 给出。
...
获取\prm{field}域\prm{item} 项的\prm{part}的任意文本注解。
如果\prm{annotatationname}未给出，
则假定 <default>(这是未显式给出名称的注解名)。可选参数\prm{field}和\prm{item}
在\cmd{getitemannotation}中时可以推断出来。\prm{part}参数无法推断，因此是必选参数。
...
日期域比较特殊，且常在\cmd{currentfield} 无法访问的环境中处理，因此需要第四个命令来访问日期的文本注解。
...
获取\prm{datetype}日期域的文本注解。如果\prm{annotatationname}未给出，
则假定 <default>(这是未显式给出名称的注解名)。
\prm{datetype} 是必须的，因为通常无法在使用 \cmd{getdateannotation} 的环境中推断出来。
...
当然还有不使用<family>姓名成分的语义上更合理的处理方式(见\secref{aut:bbx:drv})，但这个示例清晰的证明了文本注解的一种用法。
...
该选项可以用来针对特定的 \prm{resource} 覆盖全局的\opt{bibencoding} 选项
...
The label is a \emph{unique} identifier for the \prm{resource}, so each label should only be used once.
...
为当前文献表计算局部化的\cmd{labelnumberwidth}, \cmd{labelalphawidth}和类似长度，而不是针对全局的全部条目。
另见\secref{use:opt:pre:gen}节的\opt{labelnumberwidth}
...
\end{ltxsyntax}
%
%To print a bibliography with a different sorting template than the global sorting scheme, use the bibliography context switching commands from \secref{use:bib:context}.
...
改变文献列表过滤器。\prm{biblistfilter} 必须是一个\cmd{DeclareBiblistFilter}定义的有效的文献列表过滤器(见\secref{aut:ctm:bibfilt})。
\end{optionlist*}
...
%The optional argument is a comma"=separated list of resources specific to the reference section. If the argument is omitted, the reference section will use the default resource list, as specified with \cmd{addbibresource} in the preamble. If the argument is provided, it replaces the default resource list. Global resources specified with \cmd{addglobalbib} are always considered. \env{refsection} environments may not be nested, but you may use \env{refsegment} environments within a \env{refsection} to subdivide it into segments. Use the \opt{section} option of \cmd{printbibliography} to select a section when printing the bibliography, and the corresponding option of \cmd{printbiblist} when printing bibliography lists. Bibliography sections are numbered starting at~\texttt{1}. The number of the current section is also written to the transcript file. All citations given outside a \env{refsection} environment are assigned to section~0. If \cmd{printbibliography} is used within a \env{refsection}, it will automatically select the current section. The \opt{section} option is not required in this case. This also applies to \cmd{printbiblist}.
 ...
 Beginning a new reference section automatically ends the active reference context (see \secref{use:bib:context}).
...
\end{ltxexample}
%
%Here we print two bibliographies, one with the default <nyt> sorting template and one with the <ydnt> sorting scheme.
这里我们打印两个参考文献表。其中一个带有默认的“nyt”排序格式，另一个则使用“ydnt”排序格式。

%To demonstrate the second type of use of bibliography contexts, we have to understand that the actual data for an entry can vary depending on the context. This is most obvious in the case of the \opt{extra*} fields like \opt{extradate} which are generated by the backend according to the order of entries \emph{after} sorting so that they come out in the expected <a, b, c> order. This clearly shows that the \emph{data} in an entry can be different between sorting schemes. If a document contains more than one bibliography list with different sorting schemes, it can happen then that the \file{.bbl} contains sorting lists with the same entry but containing different data (a different value for \bibfield{extradate}, for example). The purpose of bibliography contexts is to encapsulate things inside a context so that \biblatex can use the correct entry data. An example is printing a bibliography list with a different sorting order to the global sorting order where the \opt{extra*} fields are different for the same entry between sorting lists:
...
Note that the \prm{string} is fully expanded, which means that you can use context-dependent macros like \cmd{thechapter}, but not unexpandable commands such as \cmd{dag}. If you need to pass unexpandable code to \prm{string}, protect it from expansion with \cmd{detokenize}.
...
该分隔符用于关联条目块前面，默认定义为\cmd{newunitpunct}。
...
用于<relatedtype>类型关联条目间的风格符，没有默认值，当未制定具体类型的分隔符时，使用\cmd{relateddelim}。
...
带星号的命令会首先清除所有 \prm{names} 中的 \prm{context} 声明。
...
将\prm{alias}声明为分隔符\prm{delim}的别名。该声明对于现有的全部文境都有效。
带星号的版本则将声明限定在给出的文境内——当可选参数为空时，声明时针对全局/空文境的。
带星号版本仅针对制定的环境设置别名。第一个可选参数\prm{alias context}保存需要设置的分隔符环境的列表。如果为空，则假定为全局环境。
第二个可选参数\prm{delim context}指定目标分隔符的环境。
如果缺失，则假定为\prm{alias context}。如果\prm{alias context}是一个列表，则会对其中每一项赋值，如果为空则使用全局环境。
...
将\opt{bib}和\opt{biblist}环境中使用的\cmd{finalnamedelim}定义为
全局环境中 \cmd{multinamedelim}的别名。
另一方面：
...
则将 \opt{bib} 环境中的\cmd{finalnamedelim}定义
为\opt{bib} 环境中的\cmd{multinamedelim} 的别名，且将\opt{biblist}环境中
的\cmd{finalnamedelim}定义
为\opt{biblist}环境中的\cmd{multinamedelim}的别名。
...
日期格式选项 \opt{iso} 则硬编码为 \cmd{slash}，因为这是标准兼容格式。
...
Note that with \bibtex as the backend, there must also be an \bibfield{entryset} field in the set members which point to the set parent. With \biber, this is not necessary.
