...
指定由 \cmd{DeclareSortingNamekeyScheme} 命令预先定义的姓名排序关键字模板(格式)。该模板用于构建文境内的姓名排序关键字。
...
然而，如果想要在不同文境（context）中打印不同的效果，这些定界符就不可取了。
...
\subsubsection{文献 filter 和标注（引用）标签}%\subsubsection{Bibliography Filters and Citation Labels}
\label{use:cav:lab}
...
本宏包生成的标注（引用）标签在被文献filter分开之前就被分配给整个文献列表。
