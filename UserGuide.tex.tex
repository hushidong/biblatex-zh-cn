...
该使用指南涵盖了要使用 \biblatex 自带标准样式所需的所有信息。
所以无论怎样都应该首先阅读该使用指南。
如果你进一步想编写你自己的引用和文献样式，请继续阅读随后的作者指南。
...
如果某个列表长度超过了该阈值，即，它包括的姓名数量超过 \prm{integer}，
...
请注意，如果与指定的区域不同，那么上下午（AM/PM）字符串对应于 \opt{abbreviate} 宏包选项。
...
类似于 \opt{time} 选项但是控制由 \cmd{DeclareLabeldate} 选择的域中得到的时刻部分格式。
...
针对 \biblatex 的实验性 XML 格式。见\apxref{apx:biblatexml}。
...
该命令重新设置引用样式。
典型应用的场景为，
样式使用 \emph{ibidem}、\emph{idem}、\emph{op.\ cit.} 等缩略语代替重复引用，
而你想在新的章节开始或别的地方强制改为引用全称。
...
这是 \TeX{} 的 \cmd{hyphenation} 命令的 \LaTeX 前端，用于定义断词例外。
\prm{language} 必须是 \sty{babel}/\sty{polyglossia} 可知的语言名。
...
该命令声明了新的本地化字符串，即为命令 \cmd{DefineBibliographyStrings} 的 \prm{definitions} 初始化新的 \prm{key}。
...
该定界符控制组成姓名成分之间的空白。
它将自动插入在名（first name）之后（如果该成分少于三个字符长度）和最后的姓名成分之前。
...
同时，\verb+\printdelim{delima}+ 在任何“textcite”之外的context 中也会打印 \cmd{delima} 也就是“Z”，
而在“textcite”中则打印“Y”。
...
本节中的命令是与语言相关的。
因此重定义时需要将新的定义包裹在 \file{.lbx} 文件的 \cmd{DeclareBibliographyExtras} 命令
或者用户文件的 \cmd{DefineBibliographyExtras} 命令中，详见 \secref{use:lng} 节。
...
对于打印 \bibfield{abstract} 和/或  \bibfield{annotation} 域的样式，
这里介绍另一种添加摘要或注释到参考文献的方法。
即，除了包含在 \file{bib} 文件中，相关文本也可以保存在外部的 \LaTeX 文件中。
...
详见 \secref{use:bib:bib, use:bib:biblist, use:bib:hdg} 节。
