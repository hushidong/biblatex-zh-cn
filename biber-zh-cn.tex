...
\subsection{文档的编码}\label{ref:enc}
...
\biber\ 满足数据源的再编码要求是非常必要的。在通常用法中，\biblatex\ 通过 \verb+.bcf+ 向\biber\ 传递\opt{bibencoding}选项值，该选项对应于 \biber 的 \opt{--input-encoding|e} 选项。\biblatex 还会传递 \opt{texencoding} 选项值(对应于 \biber 的\opt{--output-encoding|-E} 选项)，该选项的默认值依赖于\TeX\ 引擎和文档使用的编码宏包(详见\biblatex\ 手册)。
...
如果数据源为非UTF-8编码，则将其转换(解码)为UTF-8
\item %Decodes \latex\ character macros in the datasource into UTF-8
将数据源中的\latex\ 字符宏解码为UTF-8字符
\item %Encodes the output so that the \verb+.bbl+ is in the encoding that \opt{--output-encoding|-E} specifies
将输出的\verb+.bbl+ 文件转换为\opt{--output-encoding|-E} 指定的编码
\item %Warns if it is asked to output to the \verb+.bbl+ any UTF-8   decoded \latex\ character macros which are not in the   \opt{--output-encoding|-E} encoding. Replaces with a suitable \latex\ macro
  当向 \verb+.bbl+ 文件输出已经解码为 UTF-8的\latex\ 字符宏时，如果该宏没有对应的 \opt{--output-encoding|-E} 编码则给出警告。并用合适的\latex\ 宏代替。
\end{enumerate}
...
一般情况下，用户无需在\biber\ 命令行中设置编码选项，因为这些通常由\biblatex\ 通过\verb+.bcf+ 文件传递进来。然而可以通过命令行来覆盖\verb+.bcf+ 文件中的设置。编码设置的优先级，按降序排列为：\\[2ex]
...
\noindent\opt{--input-encoding|-e} 和 \opt{--output-encoding|-E} $\rightarrow$\\
\hspace*{1em}\biber\ 配置文件 $\rightarrow$\\
\hspace*{2em}\verb+.bcf+ 控制文件

%\subsubsection{\latex\ macro decoding}\label{ldecode}
\subsubsection{\latex\ 宏解码}\label{ldecode}
...
如上所述，\biber\ 总是做尽可能多的转换，包括将\latex\ 字符宏转换为 UTF-8 。这之所以重要主要有两个原因。首先，这将允许出现如下例一般的情形：
...
这里，因为\biber 将宏解码为UTF-8，所以它知道例中的两本书相同的，很显然转换后两本书的作者名是一样的。

其次，这也允许\biber 输出规范的latex 宏，当用户设置 \opt{--output-encoding=ascii} 等的时候。这意味着\biblatex 样式中使用的很多做比较用途的宏可以对包含宏的域进行可靠地比较。宏向UTF-8的转换使用 \opt{--decodecharsset} 选项指定的解码集，见下文。如果要禁止所有的宏转换为UTF-8 ，你可以指定一个空集 «null» 作为\opt{--decodecharsset} 选项的值，或者使用\opt{output-safecharsset} 选项。前者用于关闭宏向UTF-8转换，后者则用于阻止输出时的utf-8编码，即它会将utf-8字符转换回latex宏。
...
如果使用PDF\latex\ 和 \verb+\usepackage[utf8]{inputenc}+ ，\biber 对内部 \latex\ 字符宏解码得到的UTF-8 可能破坏\verb+inputenc+ 。这是因为 \verb+inputenc+ 未实现全部UTF-8，而是只有一个常用子集。
...
举个例子：如果你的\verb+.bib+ 数据源中存在有\verb+\DJ+ 字符，\biber\ 将其正确解码为 «Đ» ，但这破坏了\verb+inputenc+ ，因为它不理解UTF-8字符。其解决方法是使用完全支持UTF-8的\TeX\ 引擎，比如\XeTeX\ 或 Lua\TeX\ ，因为这些引擎不需要 \verb+inputenc+ 。然而，你也可以使用 \opt{--output-safechars} 选项，使其在输出时将UTF-8 字符转换为\latex\ 宏。关于\opt{--output-safechars} 的更多信息，见 \ref{lencode} 节。
\bigskip

%\subsubsection{\latex\ macro encoding}\label{lencode}
\subsubsection{\latex\ 宏的编码}\label{lencode}
...
解码的反面就是编码，即把UTF-8字符转换为\latex\ 宏。你可以利用 \opt{--output-safechars} 来强制实现这种转换，该选项一般工作得相当好，能确保 \verb+.bbl+ 是纯 ASCII 文档。这在一些特殊情况会很有用，比如文献中引入了主文档不能处理的字符的时候，示例见上面的\ref{ldecode} 节。
...
\latex\ 宏编码的常用情形是，文献数据源是非ASCII编码的，但\verb+.tex+ 文件是，如果做合适的设置，这中情况即可自动处理：\biblatex\ 选项 «\opt{texencoding}» (对应\biber\ 选项 «\opt{--output-encoding|-E}»)设置为ASCII编码(«\verb+ascii+» or «\verb+x-ascii+»)，且 «\opt{--input-encoding|-e}» 是非ASCII的，则\biber\ 将自动设置\opt{--output-safechars}。
...
因为 \biber 总是会在内部做UTF-8 解码，如果 \opt{--output-encoding|-E} 不设置为 UTF-8 ，\biber 会把无法编码成指定输出编码的字符自动替换为等价的\tex 宏。这也将会给出警告。
...
关于\opt{--output-safecharsset} 和 \opt{--decodecharsset}(可用于定制要使用的转换规则集)，另可参考\opt{biber --help}输出。编码和解码过程中\biber\ 做内部映射的字符和宏的集见文档\footnote{\url{https://sourceforge.net/projects/biblatex-biber/files/biblatex-biber/\biberversion/documentation/utf8-macro-map.html}}。
...
也可以使用\opt{--recodedata}选项来提供一个自定义的编码和解码映射文件。但必须遵守用于再编码的默认数据文件即\file{recode\_data.xml} 的格式，该文件具有与 \biber 的 \file{Recode.pm} 模块相同的perl安装路径。当然，通过\biber\ 源文件树能更容易的找到它们。更可能的情况是，当你需要使用一个自定义的映射文件时，可以复制默认文件并编辑它，删除相同的内容，然后定义一些定制的编码集配合 \opt{--output-safecharsset} 和 \opt{--decodecharsset} 使用。
...
在XML文件中使用正确的 «type» 属性来分类项的时候要小心，因为这将决定执行替换的代码处理宏的方式。如果需要添加新的条目，只需要从默认编码数据文件中复制一个相似类型的宏，因为该文件相当复杂所以添加新项比较困难。还要注意另外一件事： «preferred» 属性告诉\biber\ 从UTF-8映射时使用指定的\latex\ 宏，因为对于一个特定的字符，可能有多种映射。当然一般也不必使用它。
...
根据配置文件或 \verb+.bcf+ 文件设置 \opt{input-encoding} and \opt{output-encoding}。

\biberex{biber --output-encoding=latin2}

%\noindent Encode the \verb+.bbl+ as latin2, overriding the
%\verb+.bcf+
将 \verb+.bbl+ 编码为latin2，覆盖 \verb+.bcf+ 中的设置。
...
自动设置 \opt{--output-safechars} 选项，强制将UTF-8字符编码为\latex\ 宏。
...
自动设置 \opt{--output-safechars} 选项，使用完整的转换集，强制将UTF-8字符编码为\latex\ 宏。
...
使用完整的转换集，强制将\latex\ 宏解码为UTF-8字符，因为bib文件中有很多字符宏，而你需要进行正确排序。
...
%macros to UTF-8.
使用一个自定义的重(再)编码数据文件，该文件定义了一个新的再编码集  «special» ，并适应该集将\latex\ 宏解码为UTF-8字符。

\biberex{biber -u}

%\noindent Shortcut alias for \opt{biber --input-encoding=UTF-8}
\opt{biber --input-encoding=UTF-8}选项的快捷方式

\biberex{biber -U}

%\noindent Shortcut alias for \opt{biber --output-encoding=UTF-8}
\opt{biber --output-encoding=UTF-8}选项的快捷方式
